...
vector<Group> getGroups(int groupMin, int groupMax, const string& groupFile) {
    clog << "\nReading groups..." << endl;
    auto start = chrono::high_resolution_clock::now();

    ifstream groupPipe(groupFile);
    vector<Group> result;
    string line;
    string groupLine;
        ...
        if (line.starts_with("tctotal=")) {
            ...
            while (getline(orbitPipe, line)) {
                if (line.starts_with("(") || line.contains("orbits")) {
                    ...
                    auto automorphisms = getAutomorphisms(generatorLines, graphs[orbitIndex], 51);
...
int main(int argc, char *argv[] ) {
    if (argc != 9 && argc != 10) {
        cerr << "Usage: <partition> <modulo> <n> <k> <groupMin> <groupMax> <minGirth> <pureGraph6> <Optional:groupfile>" << endl;
        return 1;
    }
    ...
    const bool pureGraph6 = atoi(argv[8]) != 0;
    string groupFile = (argc == 10 && strlen(argv[9]) > 0)
                        ? argv[9]
                        : "./data/groups/groups_1-50.txt";
    ...
    clog << "pureGraph6: " << pureGraph6 << endl;
    clog << "groupFile: " << groupFile << endl;
    ...
    vector<Group> groups = getGroups(groupMin, groupMax, groupFile);
